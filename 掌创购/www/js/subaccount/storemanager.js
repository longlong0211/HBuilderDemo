...
function loadSubAccountList(pendstyle) {
    ...
    $.ajax({
        type: "POST",
        async: true,
        url: configManager.RequstUrl + "api/store/sublist",
        data: postdata
    }).done(function (data) {
        ...
        for (var i = 0; i < sublist.length; i++) {
            ...
            html.push("<li class='img-part2'> <img src='" + configManager.avatarImgurl.format(subaccount.avatar_id, "50-50") + "' width='50' height='50'></li>");
            html.push("<li class='tips-part t-c '><div class='off-right-block' name='btnoperat' tip='" + subaccount.id + "'>" + status + "</div></li>");
            html.push("<li class='text-part-70'><p>" + subaccount.nick + "</p><p>" + subaccount.real_name + "</p><p>" + subaccount.name + "</p></li> </ul></div>");
        }
...
function BindEvent() {
    ...
    $("#addsubaccount").on("click", function () {
        clicked('add.html?type=s', false, false, 'slide-in-right');
    });
}
